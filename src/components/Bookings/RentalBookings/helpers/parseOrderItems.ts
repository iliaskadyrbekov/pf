...
export const parseOrderItems = (orderItems?: IRentalOrderItem[]) => {
  ...
  const { resources, events } = orderItems.reduce<TReduceResult>(
    (acc, orderItem) => {
      ...
      const eventQuantity = orderItem.event.quantity;
          ...
          [eventKey]: {
            ...(acc.events[eventKey]
              ...
              : {
                  ...
                  quantity: isDefined(eventQuantity) ? eventQuantity : 'Unlimited',
