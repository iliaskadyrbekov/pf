...
import { useNewsPresignedContentImageUploadUrl, useNewsPresignedHeadImageUploadUrl } from '../../mutations';

const MainInfo = () => {
  const { shop } = React.useContext(ShopContext);

  const { mutate: getUploadHeadImageUrl } = useNewsPresignedHeadImageUploadUrl();
  const { mutate: getUploadContentImageUrl } = useNewsPresignedContentImageUploadUrl();
  ...
  const handleGetUploadHeadImageUrl = React.useCallback(
    async (file: File) => {
      ...
      return data?.newsPresignedHeadImageUploadUrl;
    },
    [shop],
  );
  ...
  const handleGetUploadContentImageUrl = React.useCallback(
    async (file: File) => {
      ...
      return data?.newsPresignedContentImageUploadUrl;
    },
    [shop],
  );
  ...
  return (
    <MainInfoLayout
      name={<FormMultiLanguageField component={Input} label="News name" name="name" />}
